  ...
  return (
    <span>
      ...
      {props.status === 'success' && (
        <BiCheck className="animate-fadeIn text-positive-9 text-[1.25em]" />
      )}
      {props.status === 'error' && (
        <BiSolidErrorCircle className="animate-fadeIn text-negative-9 text-[1.25em]" />
      )}
    </span>
  )
}
    ...
    >
      {(shouldSwitchChain || ['success', 'error'].includes(mutationSwitchChain.status)) && (
        ...
        >
          ...
          <span
            data-label
            className={mutationSwitchChain.status === 'pending' ? 'animate-pulse' : ''}
          >
      ...
      {transactions.map((tx) => {
        return (
          <li
            key={tx.id}
            className={labelActionLine({
              status: tx.mutationStatus === 'idle' ? 'idle' : 'default',
            })}
          >
